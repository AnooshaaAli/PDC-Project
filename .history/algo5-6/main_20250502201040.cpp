...
vector<int> find_seed_candidates() {
    ...
    for (const auto &[v, _] : IP) {
        ...
        while (!q.empty()) {
            ...
            for (int nbr : graph[u]) { // ✅ Fix: use graph[u] to go forward
        ...
        // Find L0 using go-as-far-as-decrease principle
        int L0 = max_level;
        ...
        // Selection condition with epsilon margin
        if ((IP[v] - IL0) > 1e-4 && IP[v] > 1e-6) {
