...
unordered_map<int, double> compute_IL(int v) {
    ...
    while (!q.empty()) {
        ...
        // Add the influence of node u to the appropriate level
        level_sum[lvl] += IP[u];
        level_count[lvl] += 1;

        // Explore neighbors
        for (int nbr : followers[u]) {
            if (!distance.count(nbr)) {  // If not visited
                distance[nbr] = lvl + 1;  // Set level of neighbor
                q.push(nbr);  // Enqueue for further exploration
            }
        }
    }

    // Calculate IL values for each level
    unordered_map<int, double> IL;
    for (const auto& [lvl, sum] : level_sum) {
        IL[lvl] = sum / level_count[lvl];  // Average influence at level
    }
...
vector<int> find_seed_candidates() {
    ...
    for (const auto& [v, _] : IP) {
        ...
        // Find maximum level reached
        int max_level = 0;
        for (const auto& [lvl, _] : IL) {
            max_level = max(max_level, lvl);
        }

        // Find L0 where influence starts to drop
        int L0 = max_level;  // Default to max level
        bool drop_found = false;  // Flag to check if a drop in influence was found
        for (int L = 0; L < max_level; ++L) {
            if (IL[L] > IL[L + 1]) {
                L0 = L;  // L0 is the level with the largest influence drop
                drop_found = true;
                break;
            }
        }

        // If no drop is found, consider the maximum level as L0
        if (!drop_found) {
            L0 = max_level;
        }
        ...
        // Debug print to inspect IL values
        cout << "\nNode " << v << ":\n";
        for (int i = 0; i <= max_level; ++i) {
            cout << "  Level " << i << " → IL = " << (IL.count(i) ? to_string(IL[i]) : "0") << endl;
        }
        cout << "  L0 = " << L0 << ", IL0 = " << IL0 << ", IP(" << v << ") = " << IP[v] << endl;

        // Now check if IP[v] > IL0 for seed selection
        if (IP[v] > IL0) {
