...
vector<int> findSeedCandidates(const Graph& G, const map<int, double>& IP) {
    vector<int> seeds;

    for (auto &[v, ip_val] : IP) {
        unordered_map<int, vector<int>> level_nodes;
        unordered_map<int, double> IL;  // IL[L] = average IP at level L
        ...
        while (!q.empty()) {
            ...
            for (int nbr : G.getDirectedNeighbors(node)) {
        ...
        // Compute IL[L] = average IP at each level
        for (auto &[level, nodes] : level_nodes) {
            double sum = 0.0;
            for (int u : nodes) {
                sum += IP.at(u);  // Use IP[u]
            }
            IL[level] = sum / nodes.size();
        }

        // Find L0 = first level where IL drops
        int maxLevel = IL.rbegin()->first;
        int L0 = maxLevel;
