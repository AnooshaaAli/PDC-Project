...
vector<int> find_seed_candidates() {
    ...
    for (const auto &[v, _] : IP) {
        ...
        unordered_map<int, double> node_influence; // To track influence of each node

        node_influence[v] = IP[v];  // The influence of the starting node is just its IP value

        while (!q.empty()) {
            auto [u, lvl] = q.front(); q.pop();
            level_sum[lvl] += node_influence[u];  // Sum the influence of nodes at this level
            level_count[lvl]++;
            level_nodes[lvl].push_back(u);

            // Traverse neighbors and propagate influence to the next level
            for (int nbr : followers[u]) {
                if (!visited.count(nbr)) {
                    visited.insert(nbr);
                    q.push({nbr, lvl + 1});
                    // Propagate the parent's influence to the child (adding parent's influence)
                    node_influence[nbr] = node_influence[u] + IP[nbr];
                }
            }
        }
        ...
        for (int L = 0; L <= max_level; ++L) {
            double avg = (level_count[L] > 0) ? level_sum[L] / level_count[L] : 0.0;
            IL.push_back(avg);
        }

        // Find the first level where the influence drops
        int L0 = -1;
        for (int L = 0; L < max_level; ++L) {
            if (IL[L] > IL[L + 1]) {
                L0 = L;
        ...
        double IL0 = IL[L0]; // final local influence

        cout << "\nNode " << v << ":\n";
        for (int i = 0; i <= max_level; ++i) {
            cout << "  Level " << i << " → IL = " << IL[i] << " (" << level_count[i] << " nodes)\n";
        }
        cout << "  L0 = " << L0 << ", IL0 = " << IL0 << ", IP(" << v << ") = " << IP[v] << endl;

        if (IP[v] > IL0) {  // Corrected condition
