...
vector<int> find_seed_candidates() {
    ...
    for (const auto &[v, _] : graph) {
        ...
        // Compute IL(L) for each level
        vector<double> IL;
        int max_level = 0;
        for (const auto &[lvl, sum] : level_sum) {
            max_level = max(max_level, lvl);
            double avg = level_count[lvl] > 0 ? sum / level_count[lvl] : 0.0;
            IL.push_back(avg);
        }

        // Apply "go-as-far-as-decrease" strategy to find L0
        int L0 = max_level;
        for (int L = 1; L < IL.size(); ++L) {
            if (IL[L] >= IL[L - 1]) {
                L0 = L - 1;
                break;
            }
        ...
        // Seed candidate decision based only on IP[v] > IL0
        if (IP[v] > IL0) {
            seeds.push_back(v);
        }
    }
