...
vector<int> find_seed_candidates() {
    ...
    for (const auto& [v, _] : IP) {
        ...
        // Find L0 such that IL[L0] > IL[L0 + 1]
        ...
        // Print out the computed IL values for each level
        cout << "\nNode " << v << ":\n";
        for (int i = 0; i <= max_level; ++i) {
            cout << "  Level " << i << " → IL = " << IL[i] << " (" << (IL.count(i) ? to_string(IL[i]) : "0") << ")\n";
        }
        cout << "  L0 = " << L0 << ", IL0 = " << IL0 << ", IP(" << v << ") = " << IP[v] << endl;

        // Selection condition for seed
        if (IP[v] > IL0) {
