...
vector<int> find_seed_candidates() {
    ...
    for (const auto &[_, _] : graph) {
        ...
        // DEBUG: print IL per level
        cout << "\nNode " << v << " Influence Trace:\n";
        for (auto &[lvl, sum] : level_sum) {
            if (level_count[lvl] > 0) {
                double il = sum / level_count[lvl];
                cout << "  Level " << lvl << " → IL = " << il << " (" << level_count[lvl] << " nodes)\n";
            }
        }

        int L0 = -1;
        double prev_IL = level_sum[0] / (level_count[0] > 0 ? level_count[0] : 1);
        for (int L = 1; level_count.count(L); ++L) {
            double curr_IL = level_sum[L] / (level_count[L] > 0 ? level_count[L] : 1);
            if (curr_IL >= prev_IL) {
                L0 = L - 1;
                break;
            }
        ...
        // If no increase found, use deepest level
        if (L0 == -1) {
            int maxL = 0;
            for (auto &[lvl, cnt] : level_count)
                if (cnt > 0 && lvl > maxL) maxL = lvl;
            L0 = maxL;
        }

        double IL0 = level_sum[L0] / (level_count[L0] > 0 ? level_count[L0] : 1);

        // DEBUG: final decision for node v
        cout << "  L0 = " << L0 << ", IL0 = " << IL0 << ", IP(" << v << ") = " << IP[v] << endl;

        if (IP[v] - IL0 > 1e-6 || IP[v] > 0.05) { // Add a threshold for IP[v]
