...
double psi[MAX_NODES][MAX_NODES]; // Edge weight matrix
int followers[MAX_NODES][MAX_NODES]; // followers[u] = nodes pointing to u
...
void initialize_graph() {
    ...
    while (fscanf(f, "%d %d", &u, &v) == 2) {
        psi[u][v] = 1.0; // Replace with actual ψ(u, v)
        out_degree[u]++;
        in_degree[v]++;
